...
- The terminal will print **SIGINT** and **SIGTSTP** when you press Ctrl + C and Ctrl + Z respetively. 
- When you are in a virtual environment, a pink **(env)** is prepended to $PS1.
- The current working directory (cwd) part of $PS1 can be truncated to be shorter if its length exceeds some specified amount.
    - The maximum allowed length of the cwd can be modified via the provided set_max_length function, for example: **$ set_max_length 59** (note: the function enforces that an odd integer number argument is provided)
- If you're in a directory with git version control, some information about the git repository is appended to $PS1.
    ...
    - You can choose whether or not to display git information by running **$ toggle_git_info**.